...
const APP_VERSION = '20260802-0076';
...
async function openPlayerAchievements(accountId,name){
  ...
  try{const rows=await supabaseRpc('orapa_player_achievements',{p_session_token:currentPlayerAccount.session_token,p_account_id:accountId});$('#achievementDetailContent').innerHTML=rows?.length?`<div class="achievement-list">${rows.map(row=>{const secret=row.visibility==='hidden',star=secret||row.visibility==='masked',date=new Date(row.unlocked_at).toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit',year:'2-digit'});return `<div class="achievement-row unlocked${secret?' hidden-achievement':''}"><div class="achievement-row-top"><span class="achievement-title-cell"><span class="achievement-status">✓</span><span class="achievement-title-text"><span class="achievement-name">${escapeHtml(row.name)}</span><small>Obtenu le ${date}</small></span></span><span class="achievement-count"></span><span class="achievement-points">${star?'<span class="achievement-star">★</span>':row.points+' pts'}</span></div><div class="achievement-detail" style="display:block">${escapeHtml(row.description)}</div></div>`;}).join('')}</div>`:'<div class="history-empty">Aucun succès visible.</div>';}catch(e){$('#achievementDetailContent').innerHTML=`<div class="account-error" style="display:block">${escapeHtml(e.message)}</div>`;}
}
