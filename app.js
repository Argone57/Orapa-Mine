...
function tutorialObservationText(result){
  ...
  const encountered=names.length===2
    ? `la gemme ${names[0]} et au moins une gemme ${names[1]}`
    : names.slice(0,-1).map(name=>`la gemme ${name}`).join(', ')+` et au moins une gemme ${names.at(-1)}`;
