...
function tutorialFixedLesson(){
  ...
  return {pieces,examples,ends:[4,7,7,10,13]};
}
...
function tutorialShowStage(){
  ...
  else if(tutorialStage===11){const name=CONFIG.PIECES[tutorialPlacementType()].label;const start=tutorialPlacementIndex===0?4:tutorialPlacementEnds[tutorialPlacementIndex-1]+1;const count=tutorialPlacementEnds[tutorialPlacementIndex]-start+1;let explanation='';if(tutorialPlacementIndex===0)explanation='Les deux ondes rouges obtenues depuis les entr&eacute;es 13 et 7 permettent de placer et d&rsquo;orienter le trap&egrave;ze.';else if(tutorialPlacementIndex===1)explanation='Ces trois ondes bleues, recoup&eacute;es avec l&rsquo;onde verte qui a aussi rencontr&eacute; cette gemme, permettent de placer le triangle bleu sans ambigu&iuml;t&eacute;.';else if(tutorialPlacementIndex===2)explanation='La coordonn&eacute;e d&eacute;couverte en haut du plateau confirme la pr&eacute;sence d&rsquo;une gemme blanche. En la recoupant avec l&rsquo;onde blanche obtenue depuis 12 et avec le blanc contenu dans le r&eacute;sultat vert clair, on peut identifier ici le losange et son emplacement.';else explanation=`Les ${count===3?'trois':'deux'} derni&egrave;res ondes, recoup&eacute;es avec les r&eacute;sultats pr&eacute;c&eacute;dents, permettent de retenir ce placement et cette orientation.`;tutorialCoach(`Place la gemme ${tutorialPlacementIndex+1} sur 5`,`${explanation}<br><br>Fais glisser <b>${name}</b> jusqu&rsquo;&agrave; cet emplacement.`);}
